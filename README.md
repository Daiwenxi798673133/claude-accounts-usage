...
| 命令 | 作用 |
|------|------|
| `/usage` | 弹框显示所有账号的用量(5h / 7d 两个窗口,外加各受限模型的周窗口——如 `Fable`,由 Anthropic 的 `limits` 动态返回,均带进度条与重置倒计时),并可直接切号与删号:`↑↓` 选择、`enter` 切换(立即生效)、`m` 标记/取消"不自动切"(被标记的账号不参与自动切号,面板中显示"不自动切")、`d` 删除账号(再按一次 `d` 确认,当前账号不可删)、`esc` 关闭 |
